...
class Vertex:
    def __init__(
        ...
        elif isinstance(goal_set, Hyperrectangle):
    ...
    def define_set_inequalities(self):
        ...
        hpoly = self.goal_set.MakeHPolyhedron()
        A, b = hpoly.A(), hpoly.b()
        self.set_inequalities += [b[i] - A[i].dot(self.xt) for i in range(len(b))]
    ...
    def define_potential(
        ...
        if specific_potential is not None:
            ...
            elif self.pot_type == PSD_POLY: # TODO: it don't have to be psd in the whole thing
                assert (
